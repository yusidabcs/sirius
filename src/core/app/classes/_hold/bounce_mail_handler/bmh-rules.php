...
function bmhBodyRules($body, /** @noinspection PhpUnusedParameterInspection */ $structure, $debug_mode = false)
{
	...
	if( preg_match("/domain\s+name\s+not\s+found/i", $body, $match) ) 
	{
	...
	} elseif ( strpos($body, 'Technical details of permanent failure') === false && preg_match("/Delivery to the following (?:recipient|recipients) failed permanently\X*?(\S+@\S+\w)/ui", $body, $match) ) {
		
		// if there are technical details, try another test-case
		...
		*     a@b.c
		*   Technical details of permanent failure:
		*   PERM_FAILURE: SMTP Error (state 9): 550 5.7.1 Your message (sent through 209.85.132.244) was blocked by ROTA DNSBL. 
		*	If you are not a spammer, open http://www.rota.lv/DNSBL and follow instructions or call +371 7019029, or send an e-mail message 
