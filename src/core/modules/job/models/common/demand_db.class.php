...
final class demand_db extends \core\app\classes\module_base\module_db {
    ...
    public function getJobSpeedyDemandDatatable()
    {
        ...
        $columns = array(
                ...
                array( 'db' => 'SUM(job_demand_master.demand)', 'as'=>'demand', 'dt' => 'demand' ),
                array( 'db' => 'SUM(allocation.total_allocation)', 'as' => 'allocation', 'dt' => 'allocation' ),
            );
        ...
        $join .= ' LEFT JOIN
						(
							SELECT 
								`job_demand_allocation`.`job_demand_master_id`,COUNT(`job_demand_allocation`.`job_demand_master_id`) as `total_allocation` 
							FROM `job_demand_allocation` 
							GROUP BY `job_demand_allocation`.`job_demand_master_id`
						) 
							as `allocation`
        		ON
