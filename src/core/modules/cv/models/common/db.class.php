<?php
namespace core\modules\cv\models\common;
/**
 * Final cv/db class.
 *
 * @final
...
final class db extends \core\app\classes\module_base\module_db {
    ...
    public function checkHashPersonalCV($address_book_id) {
        $out = array();
        ...
        $stmt->bind_result($address_book_id, $hash, $template);
        $stmt->execute();
        $stmt->store_result();
        while ($stmt->fetch()) {
            $out[] = array(
    ...
    public function insertHashPersonalCV($data){
        $sql = "INSERT INTO 
            `personal_cv`
        SET
            `address_book_id` = ?,
        ...
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("iss", $data['address_book_id'],$data['hash'], $data['template']);
        $stmt->execute();
        

    }

    public function updateAddressBookFileCV($address_book_id){
        $sql = "UPDATE
            `address_book_file`
        SET
            `public` = 1
    ...
    public function updateHashPersonalCV($data){
        $sql = "UPDATE
            `personal_cv`
        SET
            `template` = ?
        ...
        ";
        $stmt = $this->db->prepare($sql);
        $stmt->bind_param("si", $data['template'],$data['address_book_id']);
        $stmt->execute();
        $out = $stmt->affected_rows;
        $stmt->close();
    ...
    public function getIDHashPersonalCV($hash) {
        $out = array();
    ...
    public function getCustomizedJobSpeedyWithDemand($data, $limit = false){
    ...
    public function sendNotificationEmailToLP($application_data, $to_name, $to_email, $from_name, $from_email) {
        ...
        //subject
        $template = $mailing_common->renderEmailTemplate('new_applicant', $application_data);

        if ($template) {
            $subject = $template['subject'];
        } else {
            $subject = 'You Have New Applicant - '.$application_data['title'].' '.$application_data['entity_family_name'];
        }
