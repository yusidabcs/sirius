$(document).ready(function () {
    ...
    const dt_table = $('#recruitments').DataTable({
        ...
        "order":[],
        "ajax": {
            "url": "/ajax/recruitment/request_job",
            "type": "POST",
            data: function (d) {
        ...
        "columnDefs": [
            ...
            },
            {
                "render": function (data, type, row) {
                    ...
                    return '<a href="/personal/home/' + row['address_book_id'] + '" >' + row.fullname + '</a> <br> ' + row.main_email + '<br>' + flag;
                },
                "targets": 1
            },
            {
            ...
            },
            {
                "render": function (data, type, row) {
                    ...
                    if(row.status == 'applied') {
                        ctrac = `<a href="#" class="p-2 white border ctrac-modal" title="Ctrack Status"
                    ...
                    let html = `<div class="d-flex justify-content-around flex-column">   
                                    ...
                                    <a href="#" class="p-2 white border pre-interview-checklist" title="Pre interview checklist"
                                        data-status="${row.status}" data-id="${row.job_application_id}">
                                        <i class="fas fa-check text-info" ></i> Checklist
                                    </a>   
                                    ${premium_service} ${ctrac}`;
                    if (row.status == 'accepted' || row.status == 'interview') {
                        html += `<a href="${prescreening_url + '/' + row.job_application_id }"  class="p-2 white border pre-screening-interview" title="Pre Screening Form"
