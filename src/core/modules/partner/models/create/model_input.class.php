...
final class model_input extends \core\app\classes\module_base\module_model_input {
	...
	protected function processPost()
	{
		...
		$data = array(
            ...
            'subcountry_data' => $countrySubCode_id,
        ...
        $out = $partner_db->insertPartner($data); //get inserted partner id'
        ...
        if($out == false){
            ...
            $this->addMessage('partner','Successfully to insert data.');
            unset($_POST);
        }
    ...
    private function _processBannerImage($address_book_id,$banner_current,$banner_base64)
    {
        ...
        if($banner_current)
        {
            ...
            $affected_rows = $address_book_db->updateAddressBookFile($filename,$address_book_id,'banner',0,'register');
