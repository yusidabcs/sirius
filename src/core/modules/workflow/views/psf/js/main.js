...

                {
                    "render": function (data, type, row) {
                        ...
                        if(row.status == 'generate_invoice'){
                            ...
                            html += `<a  class="btn-sm btn-warning btn-pay-invoice" href="#" title="Pay Invoice" ><i class="fa fa-dollar-sign"></i> Pay</a>`;
                        }
    ...
    $('#generate-invoice-form').on('submit', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax/workflow/psf/generate_invoice',
                    data: $(this).serialize(),
                    success: rs => {
                        Swal.fire({
                            icon: 'success',
    ...
    $('#pay-invoice-form').on('submit', function (e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ajax/workflow/psf/pay_invoice',
                    data: $(this).serialize(),
                    success: rs => {
                        Swal.fire({
                            icon: 'success',
