$(document).ready(function () {
    ...
    const table = $('#list_vaccine').DataTable({
            "processing": true,
            "serverSide": true,
            'responsive': true,
            "ajax": {
    ...
    $(document).on('click', '.btn-review-file', function() {
        ...
        showLoadingModal();
        $.post('/ajax/workflow/vaccine/file-preview/'+data.address_book_id, { vaccination_type: data.vaccination_type }, function(response) {
            Swal.close();
            $('#vaccineModal').modal('show');
            $('#institution').html(response[0].institution);
            $('#doctor').html(response[0].doctor);
            $('#vaccination_number').html(response[0].vaccination_number);
            $('#vaccination_date').html(response[0].vaccination_date);
            $('#vaccination_expiry').html(response[0].vaccination_expiry);

            $('.confirm-vaccine, .reject-vaccine').attr('data-vaccine-id', response[0].vaccination_id);
