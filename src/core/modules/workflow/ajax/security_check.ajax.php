...
final class security_check extends \core\app\classes\module_base\module_ajax {
    ...
    public function run()
    {
        if ($this->option)
        {
            ...
            $this->security_check_db = new \core\modules\workflow\models\common\security_check_db();
            $this->common = new \core\modules\workflow\models\common\common();
            $type = $this->option;

            if ( $type == 'list' ){

                $result = $this->security_check_db->getInterviewSecurityCheckDatatable();
                ...
                foreach ($job_application_id as $index => $id){
                    $result = $this->common->sendRequestFile($id);
                    if($result){
                        $this->security_check_db->updateInterviewSecurityCheckRequestFile($id);
                    }
                }
                ...
                $interview_security_check = $this->security_check_db->getInterviewSecurityCheck($job_application_id);
                ...
                $this->security_check_db->updateInterviewSecurityCheckRequestedFile($job_application_id,$filename);
                ...
                $job_application_id = $_POST['job_application_id'];
                $result = $this->common->sendRequestClearance($job_application_id);
    ...
    private function _updateClearanceFile($data){
        ...
        foreach ($data['job_application_id'] as $job_application_id){
            $this->security_check_db->updateInterviewSecurityCheckClearanceFileStatus($job_application_id,$fm['file_obj']['file_manager_id'], $data['status']);
