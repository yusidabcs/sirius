 ...
 */
final class principal extends \core\app\classes\module_base\module_ajax {
    ...
    {
        $out = null;
        $this->finance_db = new \core\modules\workflow\models\common\principal_tracker_db();
        switch($this->option)
        {
            case 'list':

                $out = $this->finance_db->getPrincipalTrackerDatatable();
            ...
            case 'generate_invoice':
                ...
                $rs = $this->finance_db->updatePrincipalGenerateInvoice($data);
                if ($rs > 0) {
                    return $this->response([
                        'message' => 'Successfully update finance principal tracker.'
                    ]);
                }
                return $this->response([
                    'errors' => ['Unsuccessfully update finance principal tracker.']
                ], 400);
            ...
            case 'pay_invoice':
                ...
                $rs = $this->finance_db->updatePrincipalPayInvoice($data);
                if ($rs > 0) {
