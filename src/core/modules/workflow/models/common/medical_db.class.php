...
final class medical_db extends \core\app\classes\module_base\module_db {
    ...
    public function updateMedicalAppointmentDue()
    {
        $sql = " UPDATE 
                    `$this->table`
                    ...
                    AND
                        (`appointment_date_on` != NULL OR `appointment_date_on` != '0000-00-00 00:00:00')
                    AND
    ...
    public function updateMedicalTrackerLevel($workflow) {
        $tracker_db = new db();
		$out = [];
        $sql =  "SELECT 
