$(document).ready(function () {
    ...
    const table = $('#list_interview_security_report').DataTable( {
            ...
            'responsive': true ,
            "ajax": {
                "url": "/ajax/interview/security_report/list",
                "type": "POST",
                data: function (d) {
                }
            },
            ...
            "columnDefs": [
                {
                    "render": function ( data, type, row ) {
                        return row['entity_family_name'] +' '+row['number_given_name'] + ' <br> '+row['main_email'];
                    },
                    "targets": 0
                },
    ...
    $('#interview_security_form').on('submit', function () {
        ...
        $.ajax({
            url: '/ajax/interview/security_report/insert',
            data: $(this).serialize(),
            type: 'POST',
            datatype : 'json',
            success: function(rs) {
    ...
    $('body').on('keyup','#search_ab',function()
    {
        ...
        timer_search_ab = setTimeout(function()
        {
            ...
            xhr = $.post('/ajax/address_book/searchaddressbooks/interview/'+searchString, {type: 'per'})
