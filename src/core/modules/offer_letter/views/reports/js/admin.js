$(document).ready(function () {
    ...
    const table = $('#list_interview_security_report').DataTable( {
            ...
            'responsive': true ,
            "ajax": {
                "url": "/ajax/offer_letter/reports/list",
                "type": "POST",
                data: function (d) {
                }
            },
    ...
    $('#interview_security_form').on('submit', function () {
        ...
        $.ajax({
            url: '/ajax/offer_letter/reports/insert',
            data: $(this).serialize(),
            type: 'POST',
            datatype : 'json',
            success: function(rs) {
    ...
    $('body').on('keyup','#search_ab',function()
    {
        ...
        timer_search_ab = setTimeout(function()
        {
            ...
            xhr = $.post('/ajax/address_book/searchaddressbooks/offer_letter_reports/'+searchString, {type: 'per'})
                .done(function (response)
                {
                    if (response.length)
                    {
                        ...
                        {
                            html += `<option value="${item.address_book_id}" data-secondary-text="${item.main_email}">${item.entity_family_name} ${item.number_given_name}</option>`
    ...
    $('body').on('click','.btn-delete', function () {
        ...
        Swal.fire({
            ...
            if(result.value)
                $.ajax({
                    url: '/ajax/interview/security_report/delete',
                    data: data,
                    type: 'POST',
                    datatype : 'json',
                    success: function(rs) {
