$(document).ready(function()
{
	...
	const b_width = $('#seaman_croppie').data('banner-width');
	const b_height = $('#seaman_croppie').data('banner-height');
	...
	$uploadCrop = $('#seaman_croppie').croppie({
		viewport: {
			width: crop_width,
			height: crop_height
		},
	...
	$('#seaman_input').on('change', function () {

		const file_choosen = $('#seaman_input').val();
	...
	$('#seaman_result').on('click', function (ev) {
		const file_choosen = $('#seaman_input').val();
		...
		if (file_choosen !== "")
		{
			$uploadCrop.croppie('result', {
				...
				resizeImage(resp, b_width, b_height).then((resp) => {
					...
					$('#seaman_base64').val(resp);
					$('#curr_img').prop('src',resp);
					$('#d_curr_img').show();
					$('#seaman_croppie_wrap').hide();
					$('#seaman_result').hide();
					document.getElementById("seaman_image").scrollIntoView();
					$('#update_crop').show();
				})
			});
		}else{
	...
	$('#update_crop').on('click',function(){
		...
		$('#seaman_result').show();
		$(this).hide();
		document.getElementById("seaman_croppie_wrap").scrollIntoView();
	})
	...
	$('button[type=submit]').click(function(){
		...
		//check image
		if(!$('#curr_img').length){
			if(!$.trim($('input[name=seaman_base64]').val()).length)
			{
				status = false;
