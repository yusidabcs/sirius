...
final class model extends \core\app\classes\module_base\module_model {
	...
	{	
		...
		{
				$seaman_id = $this->page_options[0];
			
		} else {
			
			$msg = "What no seaman specified! How did that happen?";
		...
		if($seaman_id == 'new')
		{
			...
			$this->seaman = $personal_db->getSeaman($seaman_id);
			if(empty($this->seaman))
			{
				$msg = "What no seaman information! How did that happen?";
				throw new \RuntimeException($msg);
			}
		}
	...
	{
		$this->view_variables_obj->setViewTemplate('seaman');
		return;
	}
	...
	{	
		...
		$this->view_variables_obj->addViewVariables('seaman',$this->seaman);
